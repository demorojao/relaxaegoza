...
export default function ProfileDetailsClient({
  ...
  const [galleryTab, setGalleryTab] = useState<'all' | 'photos' | 'verified' | 'videos'>('all');
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex-1 w-full space-y-6">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {profile.avg_rating && Number(profile.avg_rating) >= 4.8 && (
              <div className="bg-gradient-to-r from-gold-primary/20 to-gold-primary/10 border border-gold-primary/40 text-gold-light px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-md" title="Excelente pontuação de satisfação dos clientes">
                ...
                <span>Top Avaliada ({Number(profile.avg_rating).toFixed(1)})</span>
              </div>
            )}
        ...
        {/* Galeria de Fotos e Vídeos (Tabbed Gallery Estilo Fatal Model) */}
        {(() => {
          const photosCount = mediaToRender.filter(m => m.type === 'photo').length;
          const verifiedCount = mediaToRender.filter(m => m.is_verified || m.type === 'photo').length;
          const videosCount = mediaToRender.filter(m => m.type === 'video').length;

          const filteredMedia = mediaToRender.filter(m => {
            if (galleryTab === 'photos') return m.type === 'photo';
            if (galleryTab === 'verified') return m.is_verified || m.type === 'photo';
            if (galleryTab === 'videos') return m.type === 'video';
            return true;
          });

          return (
            <div className="bg-black/40 border border-white/10 rounded-2xl p-5 space-y-5 shadow-xl">
              {/* Header */}
              <div className="flex items-center justify-between border-b border-white/10 pb-3.5">
                <div className="flex items-center gap-2.5">
                  <div className="p-2 rounded-xl bg-gold-primary/10 border border-gold-primary/20 text-gold-primary">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-gray-400 font-light">
                      {mediaToRender.length} {mediaToRender.length === 1 ? 'mídia publicada' : 'mídias publicadas'}
                    </span>
                  </div>
                </div>
              </div>

              {/* Abas de Navegação (Todas | Fotos | Rosto | Vídeos) */}
              <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-2 scrollbar-none border-b border-white/10">
                {/* Aba Todas */}
                <button
                  type="button"
                  ...
                  className={`px-3.5 py-2 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all cursor-pointer whitespace-nowrap ${
                    galleryTab === 'all'
                      ? 'bg-gold-primary text-dark-bg font-bold shadow-md'
                      : 'bg-white/5 text-gray-400 hover:text-white hover:bg-white/10 border border-white/5'
                  }`}
                >
                  <Grid className="w-3.5 h-3.5" />
                  <span>Todas ({mediaToRender.length})</span>
                </button>
                ...
                {/* Aba Rosto / Verificadas */}
                <button
                  type="button"
                  onClick={() => setGalleryTab('verified')}
                  className={`px-3.5 py-2 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all cursor-pointer whitespace-nowrap ${
                    galleryTab === 'verified'
                      ? 'bg-gold-primary text-dark-bg font-bold shadow-md'
                      : 'bg-white/5 text-gray-400 hover:text-white hover:bg-white/10 border border-white/5'
                ...
                >
                  ...
                  <span>Vídeos ({videosCount})</span>
                </button>
              </div>

              {/* Grid de Mídia */}
              {filteredMedia.length === 0 ? (
                <div className="text-center py-12 bg-white/2 rounded-xl border border-white/5 space-y-2">
                  <Camera className="w-8 h-8 text-gray-600 mx-auto" />
                  <p className="text-xs text-gray-400">Nenhuma mídia cadastrada nesta categoria.</p>
                </div>
              ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                  {filteredMedia.map((m, idx) => {
                    const isVideo = m.type === 'video';
                    const globalIdx = mediaToRender.findIndex(item => item.url === m.url);
                    const actualIdx = globalIdx >= 0 ? globalIdx : 0;
                    ...
                    return (
                      <div
                        ...
                        className="group relative aspect-[3/4] rounded-xl overflow-hidden border border-white/10 bg-black/40 cursor-pointer hover:border-gold-primary/50 transition-all shadow-md protected-media"
                      >
                        <div className="protected-overlay" onContextMenu={(e) => e.preventDefault()} />
                        ...
                        {isVideo ? (
                          ...
                          <Image
                            src={getCDNUrl(m.url) || '/avatar-placeholder.svg'}
                            alt={`${profile.name} - Galeria`}
                            fill
                        ...
                        {isVideo && (
                          <div className="absolute inset-0 bg-black/30 group-hover:bg-black/10 transition-colors flex items-center justify-center pointer-events-none">
                            <div className="w-10 h-10 rounded-full bg-gold-primary/80 text-dark-bg flex items-center justify-center pl-0.5 shadow-lg group-hover:scale-110 transition-transform">
                              <Play className="w-5 h-5 fill-dark-bg" />
                            </div>
                            <span className="absolute bottom-2 left-2 text-[9px] font-bold bg-black/70 text-gold-light border border-gold-primary/30 px-2 py-0.5 rounded-full flex items-center gap-1">
                              <Video className="w-2.5 h-2.5" /> Vídeo
                            </span>
                          </div>
                        )}

                        {/* Badge Foto Real / Verificada */}
                        {!isVideo && m.is_verified && (
                          <div className="absolute top-2 left-2 bg-emerald-500/80 text-dark-bg px-2 py-0.5 rounded-full text-[9px] font-bold flex items-center gap-1 shadow-md pointer-events-none">
                            <CheckCircle className="w-2.5 h-2.5" /> Foto Real
                          </div>
                        )}

                        {/* Overlay de Ampliar (Hover) */}
                        <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-2.5 pointer-events-none">
                          <span className="text-[10px] text-white font-semibold flex items-center gap-1">
                            <Eye className="w-3 h-3 text-gold-primary" /> Visualizar em HD
                          </span>
                        </div>
                      </div>
                    );
