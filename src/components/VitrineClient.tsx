...
export default function VitrineClient({ 
  ...
  // Pré-calcula metadados uma única vez por perfil para máxima performance (evita instanciação de centenas de objetos Date durante o sort)
  const sortProfiles = (profilesList: Profile[], coords: [number, number] | null) => {
    const nowTime = Date.now();

    const precomputed = profilesList.map(p => {
      ...
      if (p.boost_expires_at) {
        const time = new Date(p.boost_expires_at).getTime();
        if (time > nowTime) {
          boostTime = time;
        }
      }
      ...
      if (p.is_available_now) {
        if (!p.available_until || new Date(p.available_until).getTime() > nowTime) {
          isAvail = 1;
        }
      }

      const rating = Number(p.avg_rating !== undefined ? p.avg_rating : 5.0);
    ...
    precomputed.sort((a, b) => {
      ...
      if (a.isBoosted !== b.isBoosted) return b.isBoosted - a.isBoosted;
      if (a.isBoosted && b.isBoosted && a.boostTime !== b.boostTime) return b.boostTime - a.boostTime;
      if (a.rating !== b.rating) return b.rating - a.rating;
      if (a.reviews !== b.reviews) return b.reviews - a.reviews;
      if (a.isAvail !== b.isAvail) return b.isAvail - a.isAvail;
      if (a.distance !== b.distance) return a.distance - b.distance;
      return b.createdAt - a.createdAt;
    });

    return precomputed.map(item => item.profile);
  };
